...
This material is derived from the [Streaming ETL pipeline](https://github.com/confluentinc/ksql/blob/master/docs/tutorials/etl.md) in the [ksqlDB github](https://github.com/confluentinc/ksql).
...
In the folder `./connectors` you find the Postgres, MongoDB, and Elasticsearch connectors downloaded from [Confluent Hub](https://www.confluent.io/hub/).
...
Finally, note that the ksqlDB server image mounts the `connectors` directory, too. 
...
For ksqlDB to be able to use the topics that Debezium created, you must declare streams over it. Because you configured Kafka Connect  with {{ site.sr }}, you don't need to declare the schema of the data for the streams, because it's inferred from the schema that Debezium writes with.
...
### Add a user into Postgres, an order/shipment in the two MongoDB collections, and see the effect in ksqlDB

If you exited from the Postgres container, start by logging into the container agains:
...
psql -U postgres-user customers
```
Insert a new row into the Postgres prompt. 
...
Insert two new rows into the Mongo prompt. 

```
use logistics

db.orders.insert({"customer_id": "3", "order_id": "27", "price": 42.27, "currency": "eur", "ts": "2020-04-03T11:20:00"})
