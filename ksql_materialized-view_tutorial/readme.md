...
This material is derived from the [Materialized cache](https://github.com/confluentinc/ksql/blob/master/docs/tutorials/materialized.md) in the [ksqlDB github](https://github.com/confluentinc/ksql).
...
MySQL requires just a bit more modification before it can work with Debezium. Debezium needs to connect to MySQL as a user that has a specific set of privileges to replicate its changelog. We already set up the `example-user` by default in the Docker Compose file. Now you just need to give it the right privileges. You can do this by logging in to the MySQL container:
